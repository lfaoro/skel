...
- Also: `query` only works with `op=gh`; passing it with `repo` but no `op` errors with "url is required for op=page".
- This sandbox cannot run nix at all: even `nix-instantiate --parse` and `home-manager switch -n` fail with "opening lock file '/nix/var/nix/db/big-lock': Permission denied" (no daemon access). Fallback for Nix changes: `nixfmt --check <file>` (full parse) and leave the dry-run to the user.
